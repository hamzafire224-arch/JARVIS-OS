...
export class SelfHealingEngine {
    ...
    private async analyzeAndRecover(context: RecoveryContext): Promise<RecoveryResult> {
        ...
        switch (classification.suggestedStrategy) {
            ...
            case 'fix_syntax':
            case 'fix_types': {
                // Inject error context into args so the agent can learn from the failure
                const fixArgs = { ...context.args, _lastError: errorMsg };
                return { recovered: true, strategy: 'retry_with_context', modifiedArgs: fixArgs };
            }
