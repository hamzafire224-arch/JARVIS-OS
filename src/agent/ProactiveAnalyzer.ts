...
/**
 * Cross-platform exec that silently catches errors instead of using Unix-only
 * shell redirects like `2>/dev/null || echo ""` which fail on Windows.
 */
async function safeExec(cmd: string, cwd: string, timeout?: number): Promise<string> {
    try {
        const { stdout } = await execAsync(cmd, { cwd, timeout: timeout ?? 15000 });
        return stdout;
    } catch {
        return '';
    }
...
export class ProactiveAnalyzer {
    ...
    async analyzeGit(): Promise<ProactiveInsight[]> {
        ...
        try {
            ...
            if (this.lastGitHash && this.lastGitHash !== trimmedHash) {
                const commitLog = await safeExec(
                    `git log --oneline ${this.lastGitHash}..${trimmedHash}`,
                    this.workingDir
                );
            ...
            const conflictCheck = await safeExec(
    ...
    async analyzeDependencies(): Promise<ProactiveInsight[]> {
        ...
        try {
            ...
            const auditOutput = await safeExec(
                'npm audit --json',
                this.workingDir, 30000
            );
            ...
            const outdatedOutput = await safeExec(
                'npm outdated --json',
                this.workingDir, 30000
            );
