...
import { SkillScanner } from '../security/SkillScanner.js';
...
export class SkillMarketplace {
    ...
    async install(skillId: string): Promise<{ success: boolean; message: string }> {
        ...
        if (skill.jarvisVersion) {
            const compat = this.checkVersionCompatibility(skill.jarvisVersion);
            if (!compat.compatible) {
                return { success: false, message: compat.reason! };
            }
        }
        ...
        if (skill.repository) {
            const downloadResult = await this.downloadSkill(skill.repository, skillPath);
            if (!downloadResult.success) {
                await fs.rm(skillPath, { recursive: true, force: true });
                return { success: false, message: `Download failed: ${downloadResult.error}` };
            }
        } else {
            // No repository — save the metadata as a local-only skill
            await fs.writeFile(
        ...
        // Security scan on downloaded code
        if (this.config.securityScanEnabled) {
            const scanResult = await this.performSecurityScan(skill, skillPath);
            if (!scanResult.safe) {
                await fs.rm(skillPath, { recursive: true, force: true });
                return {
                    success: false,
    ...
    /**
     * Install a skill directly from a URL (tarball, zip, or git repo)
     */
    async installFromUrl(url: string, skillId?: string): Promise<{ success: boolean; message: string }> {
        const id = skillId ?? basename(url).replace(/\.(tar\.gz|zip|git)$/i, '');

        if (this.installedSkills.has(id)) {
            return { success: false, message: 'Skill already installed' };
        }

        const skillPath = join(this.config.skillsDir, id);
        await fs.mkdir(skillPath, { recursive: true });

        // Download
        const downloadResult = await this.downloadSkill(url, skillPath);
        if (!downloadResult.success) {
            await fs.rm(skillPath, { recursive: true, force: true });
            return { success: false, message: `Download failed: ${downloadResult.error}` };
        }
        ...
        if (existsSync(manifestPath)) {
            try {
                ...
                await fs.rm(skillPath, { recursive: true, force: true });
                return { success: false, message: 'Invalid skill.json manifest' };
            }
        } else {
        ...
        // Security scan
        if (this.config.securityScanEnabled) {
            const scanResult = await this.performSecurityScan(metadata, skillPath);
            if (!scanResult.safe) {
                await fs.rm(skillPath, { recursive: true, force: true });
                return {
                    success: false,
                    message: `Security scan blocked install: ${scanResult.reason}`
                };
            }
        }
        ...
        logger.info('Skill installed from URL', { id, url });

        return {
            success: true,
            message: `Successfully installed ${metadata.name} v${metadata.version} from URL`
        };
    }
    ...
    private async performSecurityScan(
        skill: SkillMetadata,
        skillPath?: string
    ): Promise<{ safe: boolean; reason?: string; findings?: any[] }> {
        // Check for verified badge — skip deep scan for trusted skills
        if (skill.verified) {
            return { safe: true };
        }
        ...
        if ((skill.rating ?? 0) < 3.0) {
            return { safe: false, reason: 'Low community rating (below 3.0)' };
        }
        ...
        // Deep code scan using SkillScanner (if we have files to scan)
        if (skillPath && existsSync(skillPath)) {
            try {
                const scanner = new SkillScanner();
                const files = await this.getCodeFiles(skillPath);

                for (const file of files) {
                    const result = await scanner.scanFile(file);

                    if (result.riskLevel === 'critical' || result.riskLevel === 'high') {
                        const dangerousFindings = result.findings
                            .filter(f => f.severity === 'critical' || f.severity === 'danger')
                            .map(f => `${f.description} (line ${f.line})`)
                            .join('; ');

                        logger.warn('Security scan blocked skill install', {
                            skill: skill.id,
                            file: basename(file),
                        ...
                        return {
                            safe: false,
                            reason: `High-risk code detected in ${basename(file)}: ${dangerousFindings}`,
                ...
                logger.warn('Security scan error (allowing install with warning)', {
                    skill: skill.id,
    ...
    /**
     * Get all .ts/.js code files in a directory (recursive)
     */
    ...
    /**
     * Download skill source from a URL
     */
    private async downloadSkill(
        ...
        try {
            // For GitHub repos, try to fetch package.json/skill.json directly
            let fetchUrl = url;

            // Normalize GitHub URLs to raw content
            if (url.includes('github.com') && !url.includes('/archive/') && !url.includes('/releases/')) {
                // Convert github.com/user/repo to raw content URL for skill.json
                const match = url.match(/github\.com\/([^/]+)\/([^/]+)/);
                if (match) {
                    fetchUrl = `https://raw.githubusercontent.com/${match[1]}/${match[2]}/main/skill.json`;
                }
            }
            ...
            if (contentType.includes('json') || fetchUrl.endsWith('.json')) {
                // JSON manifest — save directly
                const content = await response.text();
                await fs.writeFile(join(destPath, 'skill.json'), content, 'utf-8');
            } else {
                // Binary content (tarball/zip) — save as archive
                const buffer = Buffer.from(await response.arrayBuffer());
                const archivePath = join(destPath, 'skill-archive');
                await fs.writeFile(archivePath, buffer);
                // Note: extraction would need tar/unzip — depends on content type
            }

            logger.info('Skill downloaded', { url: fetchUrl, destPath });
            return { success: true };
        } catch (error) {
            return {
                success: false,
    ...
    /**
     * Check if skill is compatible with current JARVIS version
     */
    private checkVersionCompatibility(
        requiredVersion: string
    ): { compatible: boolean; reason?: string } {
        const reqMajor = parseInt(requiredVersion.split('.')[0] ?? '0', 10);
        const curMajor = parseInt(JARVIS_VERSION.split('.')[0] ?? '0', 10);

        if (reqMajor !== curMajor) {
            return {
                compatible: false,
                reason: `Skill requires JARVIS v${requiredVersion} but current version is v${JARVIS_VERSION} (major version mismatch)`,
            };
        }
