...
// ═══════════════════════════════════════════════════════════════════════════════
// PostgreSQL Provider (Dynamic — requires `pg` package: npm install pg)
// ═══════════════════════════════════════════════════════════════════════════════
...
export class PostgreSQLProvider implements DatabaseProvider {
    ...
    constructor(connectionString: string, readonly_ = true, maxRows = 100) {
    ...
    async connect(): Promise<void> {
        try {
            // @ts-ignore — pg is an optional dependency, resolved at runtime
            ...
            logger.debug('PostgreSQL connected', {
                host: this.connectionString.replace(/\/\/.*@/, '//***@'),
            });
        } catch (error: any) {
    ...
    async query(sql: string, params: unknown[] = []): Promise<QueryResult> {
        ...
        // Enforce readonly: block mutating statements
        if (this.readonly) {
            const upperSql = sql.trim().toUpperCase();
            const mutating = ['INSERT', 'UPDATE', 'DELETE', 'DROP', 'ALTER', 'TRUNCATE', 'CREATE'];
            if (mutating.some(kw => upperSql.startsWith(kw))) {
                return { success: false, error: `Read-only mode: ${upperSql.split(' ')[0]} not allowed` };
            }
        }
        ...
        try {
            ...
            if (upperSql.startsWith('SELECT') && !upperSql.includes('LIMIT')) {
                limitedSql = `${sql.replace(/;?\s*$/, '')} LIMIT ${this.maxRows}`;
            }
            ...
            return {
                ...
                truncated: (result.rows?.length ?? 0) >= this.maxRows,
            };
        } catch (error) {
            return {
                success: false,
    ...
    async listTables(): Promise<TableInfo[]> {
        ...
        return result.rows.map(row => ({
            name: row['name'] as string,
            type: (row['type'] as string) === 'BASE TABLE' ? 'table' as const : 'view' as const,
            columns: [],
        }));
    }
