 ...
 * - Web search via Brave Search API (with graceful fallback)
 */
...
export class ReadWebpageSkill extends Skill {
    ...
    async execute(
        ...
        return this.safeExecute(async () => {
            ...
            let content = this.extractContent(html, extractType, maxLength);

            // Playwright fallback: if regex extraction yields very little text,
            // the page is likely JS-rendered (SPA). Try Playwright if available.
            if (content.wordCount < 50 && extractType !== 'links' && extractType !== 'headings') {
                const playwrightContent = await this.tryPlaywrightFallback(url, extractType, maxLength);
                if (playwrightContent && playwrightContent.wordCount > content.wordCount) {
                    content = playwrightContent;
                    logger.tool(this.name, 'Used Playwright fallback for JS-heavy page', { url });
                }
            }
    ...
    /**
     * Try to render the page with Playwright for JS-heavy content.
     * Returns null if Playwright is not installed or fails.
     */
    private async tryPlaywrightFallback(
        url: string,
        extractType: string,
        maxLength: number
    ): Promise<Omit<WebpageContent, 'url'> | null> {
        try {
            // Dynamic import — only works if playwright is installed
            ...
            try {
                ...
                return this.extractContent(renderedHtml, extractType, maxLength);
            } finally {
                await browser.close();
            }
        } catch {
            // Playwright not installed or failed — silent fallback
            return null;
        }
    }
...
// ═══════════════════════════════════════════════════════════════════════════════
// Web Search Skill (Brave Search API + fallback)
// ═══════════════════════════════════════════════════════════════════════════════
...
export class WebSearchSkill extends Skill {
    ...
    async execute(
        ...
        if (!apiKey) {
            logger.tool(this.name, 'Web search skipped — no API key', { query });
            return {
                results: [],
                query,
                totalResults: 0,
                note: 'Web search requires BRAVE_SEARCH_API_KEY in your .env file. Get a free key at https://brave.com/search/api/',
            };
        }
        ...
        return this.safeExecute(async () => {
            ...
            const url = `https://api.search.brave.com/res/v1/web/search?q=${encodeURIComponent(searchQuery)}&count=${Math.min(maxResults, 20)}`;
            ...
            try {
                ...
                logger.tool(this.name, 'Web search complete', {
                    query: searchQuery,
                ...
                return {
                    ...
                    note: results.length === 0 ? 'No results found for this query.' : '',
                };
            } catch (error) {
                clearTimeout(timeoutId);
                if (error instanceof Error && error.name === 'AbortError') {
                    throw new ToolExecutionError(this.name, 'Search request timed out after 15s');
                }
                throw error;
            }
        }, 'web_search');
