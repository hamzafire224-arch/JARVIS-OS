...
export class TelegramAdapter extends EventEmitter {
    private config: Required<Omit<TelegramConfig, 'webhookUrl' | 'webhookSecretToken'>> & { webhookUrl?: string; webhookSecretToken?: string };
    ...
    private async setupWebhook(): Promise<void> {
        ...
        // SECURITY: Set secret_token so Telegram includes it in X-Telegram-Bot-Api-Secret-Token header
        if (this.config.webhookSecretToken) {
            webhookParams['secret_token'] = this.config.webhookSecretToken;
        }

        const result = await this.callApi('setWebhook', webhookParams);
        ...
        logger.gateway('Telegram webhook configured', {
            url: this.config.webhookUrl,
            hasSecretToken: !!this.config.webhookSecretToken,
        });
    }

    /**
     * Handle incoming webhook update (call this from your HTTP handler).
     * @param update - The update payload from Telegram
     * @param secretTokenHeader - The value of the X-Telegram-Bot-Api-Secret-Token header
     */
    async handleWebhookUpdate(update: unknown, secretTokenHeader?: string): Promise<void> {
        // SECURITY: Verify the secret token header matches our configured token
        if (this.config.webhookSecretToken) {
            if (secretTokenHeader !== this.config.webhookSecretToken) {
                logger.error('Telegram webhook secret token mismatch — rejecting update');
