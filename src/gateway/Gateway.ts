...
export class Gateway extends EventEmitter {
     ...
     * Used by JarvisHandlers to check auth requirements for terminal access.
     */
    ...
    /**
     * Send a JSON-RPC message to a specific session by ID.
     * Used by terminal handlers to stream output to the requesting session.
     */
    sendToSession(sessionId: string, message: GatewayMessage): void {
        const session = this.sessions.get(sessionId);
        if (session) {
            this.sendMessage(session, message);
        }
