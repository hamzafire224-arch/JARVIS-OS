...
Make it reflective and forward-looking.`,
        ...
        logger.gateway('Registered preset heartbeat tasks', {
            count: 5,
            autoEnabled: autoEnableBriefing ? ['morning-briefing'] : [],
        });
    }

    // ─────────────────────────────────────────────────────────────────────────────
    // Preset Discovery & Toggle
    // ─────────────────────────────────────────────────────────────────────────────

    /**
     * List all registered preset tasks with their current status
     */
    listPresets(): Array<{
        id: string;
        name: string;
    ...
    /**
     * Enable a preset task by its ID or name
     */
    enablePreset(idOrName: string): boolean {
        return this.togglePreset(idOrName, true);
    }

    /**
     * Disable a preset task by its ID or name
     */
    disablePreset(idOrName: string): boolean {
        return this.togglePreset(idOrName, false);
    }

    private togglePreset(idOrName: string, enabled: boolean): boolean {
        // Try by ID first, then by name
        let task = this.tasks.get(idOrName);
        if (!task) {
        ...
        return this.setTaskEnabled(task.id, enabled);
    }
}
