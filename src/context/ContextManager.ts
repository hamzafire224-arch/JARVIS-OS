...
${this.memory}
    ...
    private trimMessages(tokensToFree: number): Message[] {
        ...
        if (trimIndex > 0) {
            // Create a user-facing summary message for trimmed content
            const trimmedCount = trimIndex;
            const summaryMessage: Message = {
                role: 'system',
                content: `⚠️ Context window optimization: ${trimmedCount} earlier messages were compacted to free up space. Key context and recent messages have been preserved. If you need information from earlier in our conversation, please ask me to recall it.`,
                timestamp: new Date(),
            };
            ...
            logger.info(`Context trimmed: ${trimmedCount} messages removed, ~${freedTokens} tokens freed`);
            return result;
        }
