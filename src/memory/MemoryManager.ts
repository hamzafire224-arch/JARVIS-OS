...
export class MemoryManager {
    ...
    async initialize(): Promise<void> {
        ...
        if (supabase) {
            try {
                // Build query with explicit user_id filtering to prevent cross-tenant data leakage
                let query = supabase
                    .from('memory_vectors')
                    .select('*');

                // SECURITY: Always filter by user_id to prevent cross-tenant memory access.
                // This is critical even with RLS when using service_role key.
                if (this.userId) {
...
export function getMemoryManager(userId?: string): MemoryManager {
    if (!memoryManager) {
        memoryManager = new MemoryManager(undefined, userId);
    }
    return memoryManager;
}

export async function initializeMemory(userId?: string): Promise<MemoryManager> {
