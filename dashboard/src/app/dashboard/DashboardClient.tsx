...
export function DashboardClient({ plan, memberSince, hasLicense, hasData, stats, recentActivity }: DashboardClientProps) {
    ...
    const [sparklines, setSparklines] = useState<{ sessions: number[]; tasks: number[]; tools: number[]; memory: number[] } | null>(null);

    useEffect(() => {
        if (!hasData) return;
        fetch('/api/telemetry/daily')
            .then(res => res.json())
            .then(data => {
                if (!data.error) {
                    setSparklines(data);
            ...
            .catch(err => console.error('Failed to fetch daily telemetry', err));
    }, [hasData]);
            ...
            <div className="stats-grid" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))' }}>
                <div className="stat-card card-glass animate-in">
                    <div>
                        ...
                        <SparklineChart data={sparklines?.sessions || Array(7).fill(0)} color="var(--accent-1)" />
                    </div>
                </div>
                ...
                <div className="stat-card card-glass animate-in">
                    <div>
                        ...
                        <SparklineChart data={sparklines?.tasks || Array(7).fill(0)} color="var(--accent-2, var(--accent-1))" />
                    </div>
                </div>
                ...
                <div className="stat-card card-glass animate-in">
                    <div>
                        ...
                        <SparklineChart data={sparklines?.tools || Array(7).fill(0)} color="var(--accent-3, var(--accent-1))" />
                    </div>
                </div>
                ...
                <div className="stat-card card-glass animate-in">
                    <div>
                        ...
                        <SparklineChart data={sparklines?.memory || Array(7).fill(0)} color="var(--accent-4, var(--accent-1))" />
                    </div>
                </div>
            </div>
