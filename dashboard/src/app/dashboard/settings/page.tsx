...
export default function SettingsPage() {
    ...
    useEffect(() => {
        const loadUser = async () => {
            ...
            if (user) {
                ...
                try { localStorage.setItem('jarvis-prefs', JSON.stringify(prefs)); } catch (err) { showError(err instanceof Error ? err.message : 'An error occurred'); }
            }
        };
        loadUser();
    }, []);
    ...
    const savePrefs = async (updates: Record<string, unknown>) => {
        ...
        } catch (err) { showError(err instanceof Error ? err.message : 'An error occurred'); }
