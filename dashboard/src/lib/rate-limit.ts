 ...
 *
 * ⚠️ SERVERLESS LIMITATION: This store uses an in-memory Map which does NOT
 * persist across Vercel/Lambda cold starts. Each new serverless instance gets
 * a fresh Map, meaning rate limits can reset. For production at scale, replace
 * with @upstash/ratelimit + @upstash/redis for a shared Redis-backed store.
 *
 * Usage:
